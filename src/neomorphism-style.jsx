...
export default function NeomorphismStyle({onNavigate}){
 ...
 const tactileDepth=(sliderValue/100).toFixed(2);
 ...
 <section className="neo-hero"><div className="neo-intro"><span>03 / NEOMORPHISM</span><h1>Soft.<br/><em>Made tactile.</em></h1><p className="neo-lead">A surface you can almost feel.</p><p>Neomorphism creates the illusion that controls are moulded from the same physical surface — raised by light, pressed by shadow.</p><div className="neo-palette"><i/><i/><i/><i/><i/></div></div><div className="neo-panel" style={{'--neo-depth':tactileDepth}}><span className="panel-label">TACTILE SYSTEM / 01</span><h2>CONTROL<br/><em>PANEL</em></h2><div className="neo-controls"><button type="button" aria-label="Light control" aria-pressed={pressed.sun} className={pressed.sun?'neo-pressed':'neo-raised'} onClick={()=>toggle('sun')}><Sun size={20}/></button><button type="button" aria-label="Power control" aria-pressed={pressed.power} className={`${pressed.power?'neo-pressed':'neo-raised'} power`} onClick={()=>toggle('power')}><Power size={20}/></button><button type="button" aria-label="Sound control" aria-pressed={pressed.volume} className={pressed.volume?'neo-pressed':'neo-raised'} onClick={()=>toggle('volume')}><Volume2 size={18}/></button></div><div className="neo-slider" style={{'--neo-value':`${sliderValue}%`}}><span/><input aria-label="Tactile depth control" type="range" min="0" max="100" value={sliderValue} onChange={(e)=>setSliderValue(Number(e.target.value))}/></div><small>TACTILE DEPTH · {sliderValue}%</small></div></section>
