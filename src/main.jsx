...
function AnatomyVisual({ type }) {
  if (type === 'anatomy-colour') return <div className="anatomy-visual colour-visual" aria-hidden="true"><span className="colour-wheel" /><b className="colour-wheel-core" /></div>;
...
function HomePage({ onNavigate }) {
  ...
  useEffect(() => { const timer = window.setInterval(() => setActive(c => (c + 1) % previews.length), 2500); return () => window.clearInterval(timer); }, []);
  const jumpIn = () => onNavigate('dashboard');
  ...
  return <main className="home-shell">
    ...
    <section className="hero"><h1>Design has a<br /><span>language.</span></h1><p className="hero-copy">Explore the visual systems behind the designs you see — and discover how colour, type, form, space and motion work together.</p><button className="jump-button" onClick={jumpIn}><span>JUMP IN</span><ArrowRight size={18} /></button></section>
    <section className="anatomy-definition" aria-label="What is Design Anatomy"><div className="definition-copy"><span className="section-kicker">WHAT IS DESIGN ANATOMY?</span><h2>A way to see<br /><span>what makes design work.</span></h2><p>A design can look simple, complex, beautiful or strange. Underneath, it is built from a series of visual decisions.</p><p><strong>Design Anatomy breaks those decisions apart, so you can understand what you're looking at — and why it works.</strong></p></div><div className="definition-process" aria-label="Design Anatomy process"><span>SEE</span><i>→</i><span>BREAK DOWN</span><i>→</i><span>UNDERSTAND</span><i>→</i><span>CREATE</span></div><div className="definition-close"><strong>Don't just look at design.</strong><span>Learn to read it.</span></div></section>
    <section className="anatomy-section" aria-label="Look closer"><div className="anatomy-intro"><span className="section-kicker">DESIGN ANATOMY</span><h2>Look closer.</h2><p>Design isn't just what you see.<br /><strong>It's a system of decisions.</strong></p></div><div className="anatomy-stage"><div className="anatomy-orbit orbit-one" /><div className="anatomy-orbit orbit-two" />{anatomy.map((item, index) => <article className={`anatomy-item ${item.className}`} key={item.name} style={{ '--i': index }}><AnatomyVisual type={item.className} /><span className="anatomy-mark">{item.mark}</span><div><h3>{item.name}</h3><p>{item.copy}</p></div></article>)}</div><p className="anatomy-note">Colour · Type · Form · Space · Motion</p></section>
    <section className="preview-section" aria-label="Live design preview"><div className="section-meta"><div><span className="section-kicker">LIVE PREVIEW</span><h2>Watch design change.</h2></div><div className="preview-counter"><strong>0{active + 1}</strong> / 0{previews.length}</div></div><div className="preview-stage">{previews.map((preview,index)=><article key={preview.name} className={`preview-card ${preview.className} ${index===active?'is-active':''}`} aria-hidden={index!==active}><div className="preview-art"><div className="art-orbit orbit-a"/><div className="art-orbit orbit-b"/><div className="art-block block-a"/><div className="art-block block-b"/><div className="art-type">Aa</div><div className="art-line"/></div><div className="preview-caption"><div><span>DESIGN LANGUAGE</span><h3>{preview.name}</h3></div><p>{preview.accent}</p></div></article>)}</div><div className="preview-dots">{previews.map((preview,index)=><button key={preview.name} className={index===active?'active':''} onClick={()=>setActive(index)} aria-label={`Show ${preview.name}`}/>)}</div><button className="jump-button secondary" onClick={jumpIn}><span>JUMP IN</span><ArrowRight size={18}/></button></section>
