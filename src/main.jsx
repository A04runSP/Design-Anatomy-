...
function App(){const[page,setPage]=useState('home');const[style,setStyle]=useState('Minimalism');useEffect(()=>{const handler=e=>{const target=e.detail;if(typeof target==='object'){setPage(target.page||'home');setStyle(target.style||'Minimalism')}else setPage(target||'home')};window.addEventListener('ryuma:navigate',handler);return()=>window.removeEventListener('ryuma:navigate',handler)},[]);const navigate=(target)=>{if(target==='view-style'){setStyle('Minimalism');setPage('view-style');return}setPage(target)};if(page==='dashboard')return <Dashboard onNavigate={navigate}/>;if(page==='library')return <Library onNavigate={navigate}/>;if(page==='view-style')return <ViewStyle style={style} onNavigate={navigate}/>;return <HomePage onNavigate={navigate}/>}
