...
export default function BrutalismStyle({onNavigate}){
 ...
 <section className="brutal-demo"><div className="brutal-label">01 / LIVE SPECIMEN</div><div className="brutal-console"><div className="console-head"><b>CONTROL // RAW</b><span>STATUS: {active?'ON':'OFF'}</span></div><div className="console-body"><div><small>COUNTER</small><strong>{String(count).padStart(2,'0')}</strong><div className="brutal-actions"><button onClick={()=>active&&setCount(c=>Math.max(0,c-1))} aria-label="Decrease counter"><Minus/></button><button onClick={()=>active&&setCount(c=>c+1)} aria-label="Increase counter"><Plus/></button></div></div><div className="brutal-switch"><button className={active?'is-active':''} onClick={()=>setActive(v=>!v)}>{active?'ON':'OFF'}</button><small>{active?'CONTROLS ACTIVE / CLICK TO SWITCH OFF':'CONTROLS LOCKED / CLICK TO SWITCH ON'}</small></div></div></div></section>
